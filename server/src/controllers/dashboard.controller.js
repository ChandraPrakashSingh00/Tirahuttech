...
exports.getDashboard = asyncHandler(async (req, res) => {
  ...
  // 1. BASIC COUNTERS
  // ------------------------------
  const [totalVisitors, totalEvents, totalSessions, totalLeads] = await Promise.all([
  ...
  const dailyEvents = await Event.aggregate([
    ...
    { $group: { _id: groupByDay("createdAt"), count: { $sum: 1 } } },
    { $sort: { _id: 1 } },
  ]);
  ...
  const referrers = await Event.aggregate([
    { $match: { eventType: "page_view" } },
    { $group: { _id: "$properties.referrer", count: { $sum: 1 } } },
    { $sort: { count: -1 } },
  ]);
  ...
  const activeSessions = await Session.countDocuments({ updatedAt: { $gte: activeSince } });

  // ------------------------------
  // 8. LEAD FUNNEL
  // ------------------------------
  const leadStages = await Lead.aggregate([{ $group: { _id: "$stage", count: { $sum: 1 } } }]);
  ...
  res.json({
    ...
    stats: {
      ...
      activity: { dailyVisitors, dailyEvents },
      pages: topPages,
      devices,
      referrers,
